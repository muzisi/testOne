...
compare_and_copy() {
    ...
    if [[ "$dir_name" == "lib" ]]; then
        find "$high_dir" -type f -name "ailpha-ext-*-dist.jar" 2>/dev/null | while IFS= read -r dist_jar; do
            ...
            target_file="$patch_dir/$dir_name/$relative_path"
            target_dir=$(dirname "$target_file")
            mkdir -p "$target_dir"
            cp "$dist_jar" "$target_file"
            echo "  Dist jar found: $dir_name/$relative_path"
        done
    fi
    ...
    find "$high_dir" -type f 2>/dev/null | while IFS= read -r high_file; do
        ...
        if [[ -f "$low_file" ]]; then
            if ! diff -q "$high_file" "$low_file" >/dev/null 2>&1; then
                # Create target directory and copy
                target_file="$patch_dir/$dir_name/$relative_path"
                target_dir=$(dirname "$target_file")
                mkdir -p "$target_dir"
                cp "$high_file" "$target_file"
                echo "  Diff found: $dir_name/$relative_path"
            fi
        fi
    done
}
