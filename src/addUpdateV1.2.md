...
└── meta_info.json
{
    ...
    "includes": [
        {"type":"upsert","name":"lib/ailpha-ext-baas-1.jar"}, ----默认不需要
        {"type":"delete","name":"lib/ailpha-ext-lib-1.jar"} 
        {"type":"delete","name":"conf/mybatis/mapper/postgresql/"}    
    ]
}
```
### 1.1 前置条件
...
FROM scratch
COPY --from=baseline /app/ /app/
RUN rm -f /usr/hdp/2.5.3.0-37/baas/baasweb/lib/ailpha-ext-baas-*-dist.jar           # 先复制基线内容
COPY lib/ailpha-ext-baas-v5.3.0.12_beta_dev-8a07a2b0_201b63b0_1.0.2-2604160630-dist.jar/ /usr/hdp/2.5.3.0-37/baas/baasweb/lib/                   # 再覆盖新版本内容
LABEL version="1.2.0"
```
